...
namespace XML2DB
{
    public partial class DataBaseToDGV : UserControl
    {
                    ...
                    foreach (DataGridViewRow row in dg_SQL.Rows)
                    {
                        foreach (DataGridViewColumn col in dg_SQL.Columns)
                        {
                            if (row.Cells[col.Index].Value.ToString().Equals(searchValue))
                            {
                                ...
                                dg_SQL.FirstDisplayedScrollingRowIndex = row.Index;
                                dg_SQL.Focus();
                                break;
                            }
                            else
        ...
        private void dg_SQL_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (!ActivateMod)
            {
                MessageBox.Show("You need to activate Edit mode to enable modification", "info", MessageBoxButtons.OK,
